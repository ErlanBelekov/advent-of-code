...
const input = `fivethreeonezblqnsfk1
two74119onebtqgnine
jrjh5vsrxbhsfour3
...
func main() {
	...
	for _, line := range inputLines {
		var dig1, dig2 int = -1, -1

		for i := 0; i <= len(line)-1 && dig1 == -1; i++ {
			if dig, err := strconv.Atoi(string(line[i])); err == nil {
				dig1 = dig
			}
		}

		for j := len(line) - 1; j >= 0 && dig2 == -1; j-- {
			if dig, err := strconv.Atoi(string(line[j])); err == nil {
				dig2 = dig
			}
		}

		digsCombined := fmt.Sprintf("%s%s", strconv.Itoa(dig1), strconv.Itoa(dig2))
